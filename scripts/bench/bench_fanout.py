...
NB_USERS_TOTAL = 1000
POSTS_PER_USER = 100  # fixé pour ce test

FANOUT_VALUES = [10, 50, 100]
NB_RUNS = 3
CONCURRENCY = 50

OUT_FILE = "/home/khalilfahler/out/fanout.csv"
USER_PREFIX = "user"
...
def fetch_timeline(username: str, timeout: float = 50.0) -> float:
...
def seed_fanout_only(fanout: int):
    # Crée uniquement les follows, en partant du principe que
    # users + posts existent déjà.
    # Utilise --skip-posts.
    total_posts = NB_USERS_TOTAL * POSTS_PER_USER
    print(
        f"[SEED FANOUT] fanout={fanout} (skip-posts)"
    )
...
def main():
    ...
    with open(OUT_FILE, "w", newline="") as f:
        ...
        for fanout in FANOUT_VALUES:
            ...
            seed_fanout_only(fanout-previous_fanout)
            # Bench (3 runs)
            for run_id in range(1, NB_RUNS + 1):
                ...
                # Écriture du CSV
                writer.writerow([fanout, f"{avg_ms:.2f}ms", run_id, failed])
            previous_fanout = fanout
